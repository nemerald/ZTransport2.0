...
public class GenerateReportFragment extends Fragment{
    ...
    private void showConfirmationDialog(final int pickedReportInterval) {
        ...
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String message="";
                ArrayList parsedArray = new ArrayList();
                if(pickedReportInterval==R.id.rbWeekReport){
                    parsedArray = prepareWeekReportToBeParsedToExcel(yearMonthDayWeekHashMap.get("year").toString() , etOtherWeek.getText().toString(), reportArrays.get(0));
                    if(!parsedArray.isEmpty()){
                        message = parseArrayToExcelAndGenerateReport(getActivity(), parsedArray, R.id.rbWeekReport);
                        setDefaultState();
                    }else{
                ...
                if(pickedReportInterval==R.id.rbMonthReport){
                    parsedArray = prepareMonthReportToBeParsedToExcel(spPickYear.getSelectedItem().toString(), spPickMonth.getSelectedItemPosition()+1, reportArrays.get(0));
                    if(!parsedArray.isEmpty()){
                        message = parseArrayToExcelAndGenerateReport(getActivity(), parsedArray, R.id.rbMonthReport);
                        setDefaultState();
                    }else{
                        message = getResources().getString(R.string.report_is_empty_for_interval);
                    }
